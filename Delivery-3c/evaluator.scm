...
(define (eval-special-form exp env)
  (cond ((quoted? exp) (text-of-quotation exp))
        ...
        ((and? exp) (eval-and (cdr exp) env)) ;; 3a
        ((or? exp) (eval-or (cdr exp) env)) ;; 3a
...
;;;
;;; Endringer i evaluatoren oppsummert:
;;;

;;; Added "and?", "or?", og "let?" til special-forms.
...
;; Sjekker først ingen argumenter er med. Velger da å evaluere til #f.
;; Looper så gjennom alle argumentene. Om det kommer en som evaluerer
;; til false, så stopper prosedyren og returnerer false. Ellers, om den
;; looper gjennom alle argumentene, så er alle true. Dette
;; fører til at "item" blir tom. Returnerer da true.
(define (eval-and exp env)
  (if (null? exp)
      #f
      (letrec ((loop (lambda (item)
                      (if (null? item)
                          #t
                          (if (true? (mc-eval (car item) env))
                              (loop (cdr item))
                              #f)))))
        (loop exp))))
...
;; Denne kan skrives litt penere, ettersom vi slipper den interne prosedyren.
;; 1. Dersom ingen argumenter, returner false. Ingen argumenter kan bety at
;;     a) ingen argumenter ble sendt med prosedyrekallet
;;     b) vi har rekursert gjennom alle argumentene
;; 2. Dersom første argument er true, returner true. Ellers gå til steg 3.
;; 3. Rekurser med resternde argumenter.
...
;; Et let-utrykk består av id, verdi, og kropp.
;; Vi trenger derfor en abstraksjonsbariere for de tre elementene, og i tillegg har vi
;; let-lamba som binder sammen delene til det fulle utrykket. 
...
(define (let-lambda exp)
  (append
   (list (make-lambda (let-id exp)(let-body exp)))
   (let-values (cadr exp))))
